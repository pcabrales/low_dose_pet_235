...
- Reads data from a Zarr store with structure:
    /RAID/mpet/PET_LOWDOSE/TRAINING_DATA/explorer_part1_pet.zarr
        <case_id>/
            clean         (D,H,W)
            noisy_<level> (D,H,W)  # e.g., noisy_D10, noisy_1-10, noisy_0.050, ...
...
def parse_sigma_from_key(key: str) -> float:
    """
    Accepts dataset keys like 'noisy_0.200', 'noisy_1-10' (interpreted as 1/10),
    or 'noisy_D10' (interpreted as 1/10 dose).
    Returns the numeric noise level as float.
    """
    suffix = str(key).split("_", 1)[1]
    suffix = suffix.replace(",", ".")
    lower = suffix.lower()
    if lower.startswith("d") and len(lower) > 1:
        try:
            denom = float(lower[1:])
            if denom == 0:
                raise ValueError("Dose denominator cannot be zero.")
            return 1.0 / denom
        except Exception as exc:
            raise ValueError(f"Cannot parse sigma from key '{key}' as dose fraction") from exc
    if "-" in suffix:
...
def main():
    ...
    ap.add_argument("--zarr", default="/RAID/mpet/PET_LOWDOSE/TRAINING_DATA/explorer_part1_pet.zarr", help="Path to Zarr dataset")
