# CURRENT ACTIVATION FUNCTIONS ARE RELU BECAUSE THEY SEEMED TO WORK BETTER FOR DECAY RATE PREDICTION (PROTOTWIN-PET-WASHOUT)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=4, embed_dim=96, norm_layer=None):
        ...
        self.proj1 = project(in_chans,embed_dim//2,stride1,1,nn.ReLU,nn.LayerNorm,False)
        self.proj2 = project(embed_dim//2,embed_dim,stride2,1,nn.ReLU,nn.LayerNorm,True)
...
class Decoder(nn.Module):
    ...
    def forward(self,x,skips):
        ...
        for idx in range(self.num_layers)[::-1]:
            ...
            # Process the skip connection without modifying it in-place
            skip = skips[idx]
            skip = skip.flatten(2).transpose(1, 2).contiguous()

            # Pass the processed skip to the layer
            x, S, H, W = layer(x, skip, S, H, W)

            # Reshape and collect the output
            out = x.view(-1, S, H, W, self.num_features[idx])
            outs.append(out)
...
class final_patch_expanding(nn.Module):
    def __init__(self,dim,num_class,patch_size, aleatoric_uncertainty=False):
        super().__init__()
        self.num_classes=num_class
        self.aleatoric_uncertainty=aleatoric_uncertainty
        if self.aleatoric_uncertainty:
            self.up=nn.ConvTranspose3d(dim,2*num_class,patch_size,patch_size)
    ...
    def forward(self,x):
        ...
        if not hasattr(self, 'aleatoric_uncertainty'):
            # If not, define it and set it to False
            self.aleatoric_uncertainty = False
        
        if self.aleatoric_uncertainty:
            logits = x[:, :self.num_classes, :, :, :]  # first half of channels are logits
            variances = self.softplus(x[:, self.num_classes:, :, :, :])  # fsecond are variances, softplus ensures positive
            x = torch.cat((logits, variances), dim=1)
        return x    
...
class nnFormer(SegmentationNetwork):
    ...
    def __init__(self, crop_size=[96,96,96],
        ...
        self.upscale_logits_ops.append(identity_function)
        ...
        ### Approach 1: SIMPLY ADDING ADDITIONAL CHANNELS FOR THE VARIANCE
        if self.aleatoric_uncertainty:
            self.final = final_patch_expanding(embed_dim,num_classes,patch_size=patch_size, aleatoric_uncertainty=self.aleatoric_uncertainty)  # output logits and variance for each class
        else:
            self.final=final_patch_expanding(embed_dim,num_classes,patch_size=patch_size)
        ###
        
        ### APPROACH 2: USING A SEPARATE BRANCH FOR VARIANCE
        # if self.aleatoric_uncertainty:
        #     self.final = final_patch_expanding(embed_dim,num_classes,patch_size=patch_size)  # output mean  
        #     self.decoder_aleatoric = Decoder(pretrain_img_size=crop_size,embed_dim=embed_dim,window_size=window_size[::-1][1:],patch_size=patch_size,num_heads=num_heads[::-1][1:],depths=depths[::-1][1:],
        #                         drop_rate=drop_rate,attn_drop_rate=attn_drop_rate)
        #     self.final_aleatoric=final_patch_expanding(embed_dim,num_classes,patch_size=patch_size)  # output aleatoric variance
        #     self.softplus = nn.Softplus()
        ###
    ...
    def forward(self, x):
        ...
        neck=skips[-1]
        decoder_out=self.decoder(neck,skips)
        # check is self.final is a nn.ModuleList and if it is apply the final elemment to the decoder_out
        ...
        ### Approach 2       
        # if self.aleatoric_uncertainty:
        #     aleatoric_decoder_out = self.decoder_aleatoric(neck, skips)
        #     aleatoric_out = self.softplus(self.final_aleatoric(aleatoric_decoder_out[-1]))
        #     out = torch.cat((out, aleatoric_out), dim=1)
        ###
        return out
