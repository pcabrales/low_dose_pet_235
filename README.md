...
Minimal MONAI DynUNet training + inference for low-dose PET denoising.
...
Notes
- This installs `torch`, `monai`, `SimpleITK`, `numpy`, and `matplotlib`.
...
Run the training + inference script directly. Specify the dose reduction factor (DRF) for the input folder name `1-<DRF> dose`:

```
python3 low_dose_pet_235/train_dynunet.py --drf 10
```
...
What it does
- Trains DynUNet on 96x96x96 patches (stride 80) with flips/rotations.
...
Quick tips
- For a short smoke test, edit `MAX_PATIENTS` near the top of `low_dose_pet_235/train_dynunet.py`.
- Ensure the expected data folders exist and contain a single DICOM/IMA series each.

## Artifacts and outputs

- Models: `low_dose_pet_235/trained-models/dynunet_<RUNID>.pt`
